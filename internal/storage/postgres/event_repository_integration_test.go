...
func TestEventRepository_InsertAndFind(t *testing.T) {
	...
	if found.Status != app.EventStatusPending {
...
func TestEventRepository_UpdateStatus(t *testing.T) {
	...
	if err := repo.UpdateStatus(ctx, ev.ID, app.EventStatusProcessed); err != nil {
		t.Fatalf("update status: %v", err)
	}

	found, _ := repo.FindByID(ctx, ev.ID)
	if found.Status != app.EventStatusProcessed {
...
func TestEventRepository_ListByStatus(t *testing.T) {
	...
	list, err := repo.ListByStatus(ctx, app.EventStatusPending, 10)
	if err != nil {
...
func TestIdempotencyPostgresStore_SetAndGet(t *testing.T) {
	db := startPostgres(t)
	ctx := context.Background()
...
func TestIdempotencyPostgresStore_MissingKeyReturnsNil(t *testing.T) {
	db := startPostgres(t)
	ctx := context.Background()

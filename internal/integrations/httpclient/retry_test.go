...
func TestBackoff_ZeroAttemptIsInstant(t *testing.T) {
	// attempt=0 → ceiling = baseDelay * 2^0 = baseDelay
	// jitter ∈ [0, baseDelay], so always ≤ baseDelay
	for i := 0; i < 50; i++ {
		d := backoff(0)
		if d > baseDelay {
...
func TestBackoff_CapsAtMaxDelay(t *testing.T) {
	// At high attempt numbers the ceiling saturates at maxDelay.
	// With full jitter the result is ∈ [0, maxDelay] — never above.
...
func TestBackoff_GenerallyIncreases(t *testing.T) {
	// Full jitter means a single sample can be low; measure average over many samples.
	const samples = 200
